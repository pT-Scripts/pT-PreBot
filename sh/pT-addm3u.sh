...
if [ "$exists" -gt 0 ]; then
    ...
    query="UPDATE $xtra_table SET m3u='$m3u', lastupdated='$current_datetime' WHERE rlsname='$release';"
else
    # Construct MySQL query to insert new record
    query="INSERT INTO $xtra_table (rlsname, m3u, lastupdated) 
           VALUES ('$release', '$m3u', '$current_datetime');"
fi
