...
if [ "$exists" -gt 0 ]; then
    ...
    query="UPDATE $xtra_table SET sfv='$sfv', lastupdated='$current_datetime' WHERE rlsname='$release';"
else
    # Construct MySQL query to insert new record
    query="INSERT INTO $xtra_table (rlsname, sfv, lastupdated) 
           VALUES ('$release', '$sfv', '$current_datetime');"
fi
