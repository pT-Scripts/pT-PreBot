...
if [ "$exists" -gt 0 ]; then
    ...
    query="UPDATE $xtra_table SET jpg='$jpg', lastupdated='$current_datetime' WHERE rlsname='$release';"
else
    # Construct MySQL query to insert new record
    query="INSERT INTO $xtra_table (rlsname, jpg, lastupdated) 
           VALUES ('$release', '$jpg', '$current_datetime');"
fi
